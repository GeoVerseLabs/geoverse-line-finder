...
`perLevel` is derived per **connector run** — one part of a connector feature from its start-group vertex to its end-group vertex — and not per compacted chain. Chain compaction merges "an expensive F1 corridor + the stairs + an F2 corridor" into one chain; deriving from that gives roughly 995 per level, while entering from the chain's F2 end towards a target snapped onto the corridor right next to the stairs really costs about 20 — the bound would no longer be admissible (the counterexample is pinned by a test). Admissibility: any path splits into same-level stretches (`cost ≥ scale·|Δe|`) and whole connector runs (`cost ≥ scale·|Δe| + perLevel·|Δord|`); summing and applying the triangle inequality in the embedding space and on the ordinals gives the bound. When the target sits inside a connector (a waypoint without a level constraint can snap onto a staircase), the chain is walked outwards to the first vertex with a group on each side, and the distance from the node's ordinal to that interval is used — equally a lower bound. Multiple targets take the minimum over targets of "geometric + level"; virtual nodes return 0.

`perLevel` degenerates to 0 in two cases (results stay correct, only the speed-up is lost): there is a free way to change level (a zero-length lift with `zeroWeight: 'free'`), or some group carries live vertices but has no `ordinal` — passing through an ordinal-less outdoor area could then look free, so **one such group switches the level term off for the whole graph**, and `diagnostics().missingOrdinals` names them.
