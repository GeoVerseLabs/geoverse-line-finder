...
`perLevel` 按**连接器段**推导——一个连接要素的一个部分从起点组顶点到终点组顶点的整段，而不是链压缩后的链。链压缩会把"很贵的 F1 走廊 + 楼梯 + F2 走廊"合成一条链，按链推导会得出约 995/层 的值，而从链的 F2 端进入、目标吸附在走廊上紧挨楼梯处时真实剩余代价只有约 20——下界就不再可采纳（反例已固化为测试）。可采纳性论证：任一路径可拆成同层线段（`cost ≥ scale·|Δe|`）与完整连接器段（`cost ≥ scale·|Δe| + perLevel·|Δord|`），求和后用嵌入空间与楼层序号上的三角不等式即得。目标落在连接器中段时（不带楼层约束的途经点可以吸附到楼梯上），沿链向两侧走到第一个有组的顶点，取两端序号构成区间，点到区间的距离同样是下界。多目标取各目标"几何项 + 楼层项"之和的最小值；虚拟节点返回 0。

两种情况下 `perLevel` 退化为 0（结果仍正确，只是不加速）：存在零代价的换层方式（`zeroWeight: 'free'` 的零长度电梯）；存在带活顶点却没有 `ordinal` 的分组——那样"经无序号的室外换层"可能看起来免费，所以**一个这样的分组就关掉整张图的楼层项**，`diagnostics().missingOrdinals` 报出它们。
