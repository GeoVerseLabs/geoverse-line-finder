...
| 搜索     | A* + 四叉堆 + 按查询打戳的 scratch 缓冲 + ALT 地标（第 256 次查询后启用）                | Dijkstra（tinyqueue），每个队列状态都拷贝整条路径数组                       | _*A* / Dijkstra 可切换_*，四叉堆与 scratch 复用取 terra-route |
...
terra-route 的 ALT 地标没有搬过来：它基于无向图，有向图需要正反两套地标距离（`d(L,v)` 与 `d(v,L)`）。已在路线图中，作为 `PathAlgorithm` 或启发式提供者接入即可，不影响现有接口。
